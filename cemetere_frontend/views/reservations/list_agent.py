"""
views/reservations/list_agent.py — Gestion des réservations (Agent).
Compatible Flet 0.86.3. L'agent PEUT confirmer l'inhumation, mais NE PEUT PAS valider/rejeter.
"""
...
def build_reservations_list_agent_view(page: ft.Page, auth: AuthState) -> ft.View:
    ...
    def _filter_style(active: bool) -> ft.ButtonStyle:
        return ft.ButtonStyle(bgcolor=Colors.PRIMARY if active else "#FFFFFF", color=Colors.TEXT_ON_DARK if active else Colors.TEXT, side=ft.BorderSide(1, Colors.PRIMARY if active else Colors.BORDER))

    def on_filter_click(filter_key: str, btn: ft.ElevatedButton):
        nonlocal current_filter
        current_filter = filter_key
        for b in filter_buttons: b.style = _filter_style(b == btn)
        page.update()
        render_list()
    ...
    def render_stats() -> None:
        def chip(label, value, color): 
            return ft.Container(content=ft.Column([ft.Text(str(value), size=18, weight=ft.FontWeight.BOLD, color=color), ft.Text(label, size=11, color=Colors.NEUTRAL)], spacing=2, horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=14, bgcolor="#FFFFFF", border_radius=10, border=ft.Border(top=ft.BorderSide(3, color)))
        a_inhumer = len([r for r in reservations if r.get("status") == "confirmed" and not _is_inhumed(r) and _is_due_today_or_past(r)])
        stats_row.controls = [chip("Total", len(reservations), Colors.PRIMARY), chip("En attente", len([r for r in reservations if r.get("status") == "pending"]), "#8B6B3F"), chip("À inhumer", a_inhumer, "#8A4343"), chip("Terminées", len([r for r in reservations if _is_inhumed(r)]), "#496042")]
    ...
    def load_reservations() -> None:
        ...
        try:
            ...
            reservations.clear()
            if isinstance(data, list): reservations.extend(data)
            elif isinstance(data, dict): reservations.extend(data.get("items", data.get("results", data.get("data", []))))
            render_stats()
            render_list()
    ...
    # ✅ DIALOGUE DE CONFIRMATION D'INHUMATION (Restauré pour l'Agent)
    def show_confirm_inhumation_dialog(res: dict):
        ...
        date_field = ft.TextField(label="Date réelle d'inhumation *", border_radius=8, value=date.today().isoformat(), hint_text="AAAA-MM-JJ")
        ...
        def on_confirm(e):
            ...
            try:
                ...
                auth.api.put(f"/reservations/{res['id']}/confirmer-inhumation", json=payload)
                dialog.open = False
                page.update()
                _snack("✅ Inhumation confirmée : caveau marqué occupé.", "#496042")
                load_reservations()
            except Exception as exc:
    ...
    def build_row(res: dict) -> ft.Control:
        ...
        # ✅ CORRECTION : L'agent NE PEUT PAS valider/rejeter (status == "pending")
        if status == "pending":
            action = ft.Text("En attente de validation", size=11, color=Colors.NEUTRAL, italic=True)
        
        # ✅ CORRECTION : L'agent PEUT confirmer l'inhumation (status == "confirmed" and not inhumed)
    ...
    def render_list() -> None:
        ...
        if not filtered:
            list_container.controls.append(ft.Container(content=ft.Text("Aucune réservation.", color=Colors.NEUTRAL, italic=True), padding=40, alignment=ft.Alignment(0.5, 0.5)))
        else:
            for res in filtered: 
